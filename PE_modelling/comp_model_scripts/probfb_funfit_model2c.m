% Model 2c: Fit model with four learning rates separately for valence and
% choice (positive fb trial chosen action, negative fb chosen, positive fb 
% unchosen, negative fb unchosen) + update of unchosen action value
...
for i = 1:nsubs
    ...
    for j = 1:niter
        ...
        alpha_pos_unchosen = rand; % separate learning rates for
        alpha_neg_unchosen = rand; % positive and negative outcomes
        beta = rand*100; % one exploration parameter
        params = [alpha_pos, alpha_neg, alpha_pos_unchosen, alpha_neg_unchosen, beta];
        ...
        [params, LL] = fmincon(@probfb_fun_model2c,params,[],[],[],[],LB,UB,[],options,......
        ...
        all_alpha_pos_unchosen(j) = params(3);
        all_alpha_neg_unchosen(j) = params(4);
        all_beta(j) = params(5);
    ...
    fit.alpha_pos_unchosen(i) = all_alpha_pos_unchosen(best);
    fit.alpha_neg_unchosen(i) = all_alpha_neg_unchosen(best);
    ...
    fit_BIC.alpha_pos_unchosen(i) = all_alpha_pos_unchosen(bestBIC);
    fit_BIC.alpha_neg_unchosen(i) = all_alpha_neg_unchosen(bestBIC);
