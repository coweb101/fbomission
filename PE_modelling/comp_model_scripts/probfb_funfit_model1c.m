% Model 1: Update of chosen + unchosen action + separate learning rates for
% chosen and unchosen
...
for i = 1:nsubs
    ...
    for j = 1:niter
        ...
        alpha_chosen = rand; % one learning rate for chosen actions
        alpha_unchosen = rand; % one learning rate for unchosen actions
        beta = rand*100; % one exploration parameter
        params = [alpha_chosen, alpha_unchosen,beta];
        
        % set options for fmincon
        options = optimset('display','off','MaxFunEvals',100000,'TolFun',1e-16,'TolX',1e-16); 
        
        % call fmincon to minimize output of function: probfb_fun_model1c
        [params, LL] = fmincon(@probfb_fun_model1c,params,[],[],[],[],LB,UB,[],options,......
            sub_choice,sub_outcome,sub_not_choice,ntrials, sub_stimuli);
        
        % save optimized parameter of this iteration of fmincon fit
        all_alpha_chosen(j) = params(1);
        all_alpha_unchosen(j) = params(2);
        all_beta(j) = params(3);
    ...
    fit.alpha_chosen(i) = all_alpha_chosen(best);
    fit.alpha_unchosen(i) = all_alpha_unchosen(best);
    ...
    % save parameter values of this fit
    fit_BIC.alpha_chosen(i) = all_alpha_chosen(bestBIC);
    fit_BIC.alpha_unchosen(i) = all_alpha_unchosen(bestBIC);
