% Model 3c: Fit model with four learning rates separately for presented
% and omitted feedback and the chosen and unchosen action (+ update of 
% unchosen action)
...
for i = 1:nsubs
    ...
    for j = 1:niter
        ...
        alpha_presented = rand; % separate learning rates for
        alpha_omitted = rand; % presented and omitted feedback
        alpha_presented_unchosen = rand; % and for chosen and unchosen
        alpha_omitted_unchosen = rand; % action
        beta = rand*100; % one exploration parameter
        params = [alpha_presented, alpha_omitted, alpha_presented_unchosen,...
            alpha_omitted_unchosen, beta];
        ...
        [params, LL] = fmincon(@probfb_fun_model3c,params,[],[],[],[],LB,UB,[],options,......
        ...
        all_alpha_presented(j) = params(1);
        all_alpha_omitted(j) = params(2);
        all_alpha_presented_unchosen(j) = params(3);
        all_alpha_omitted_unchosen(j) = params(4);
        all_beta(j) = params(5);
    ...
    fit.alpha_presented(i) = all_alpha_presented(best);
    fit.alpha_omitted(i) = all_alpha_omitted(best);
    fit.alpha_presented_unchosen(i) = all_alpha_presented_unchosen(best);
    fit.alpha_omitted_unchosen(i) = all_alpha_omitted_unchosen(best);
    ...
    % save parameter values of this fit
    fit_BIC.alpha_presented(i) = all_alpha_presented(bestBIC);
    fit_BIC.alpha_omitted(i) = all_alpha_omitted(bestBIC);
    fit_BIC.alpha_presented_unchosen(i) = all_alpha_presented_unchosen(bestBIC);
    fit_BIC.alpha_omitted_unchosen(i) = all_alpha_omitted_unchosen(bestBIC);
