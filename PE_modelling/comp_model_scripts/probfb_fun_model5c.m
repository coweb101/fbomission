function error = probfb_fun_model5c(params,sub_choice,sub_fb,sub_not_choice,ntrials, sub_stimuli, sub_feedback_type)

% Separate learning rates for valence + appearance + choice (+ unchosen
% update)
...
alpha_pospresented_unchosen = params(5);
alpha_posomitted_unchosen = params(6);
alpha_negpresented_unchosen = params(7);
alpha_negomitted_unchosen = params(8);
beta = params(9);
...
for i = 1:ntrials
    ...
    if isnan(sub_choice(i)) || isnan(sub_fb(i))
        ...
        if sub_feedback_type(i) == 1 & sub_fb(i) == 1 
            ...
            % update action value of chosen action with alpha_pospresented
            Q(chosen_index) = Q(chosen_index) + alpha_pospresented*PE_c;
            Q(unchosen_index) = Q(unchosen_index) + alpha_pospresented_unchosen*PE_u;
            
        elseif sub_feedback_type(i) == 0 & sub_fb(i) == 0
        % else if feedback is omitted and feedback is negative
        
            % use alpha_negomitted to update action value of chosen
            Q(chosen_index) = Q(chosen_index)+ alpha_negomitted*PE_c;
            Q(unchosen_index) = Q(unchosen_index) + alpha_negomitted_unchosen*PE_u;
        ...
        % else if feedback is omitted and feedback is positive
            ...
            Q(unchosen_index) = Q(unchosen_index) + alpha_posomitted_unchosen*PE_u;
        ...
        % else if feedback is presented and feedback is negative
            ...
            Q(unchosen_index) = Q(unchosen_index) + alpha_negpresented_unchosen*PE_u;
        end
