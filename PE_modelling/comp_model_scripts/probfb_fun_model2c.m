function error = probfb_fun_model2c(params,sub_choice,sub_fb,sub_not_choice,ntrials, sub_stimuli)

% Separate learning rates for positive and negative FB, and chosen and
% unchosen action (+ Update of Unchosen)

% set parameters
alpha_pos = params(1);
alpha_neg = params(2);
alpha_pos_unchosen = params(3);
alpha_neg_unchosen = params(4);
beta = params(5);
...
for i = 1:ntrials
    ...
    if isnan(sub_choice(i)) || isnan(sub_fb(i))
        ...
        % save in temporal variable q_i the current action values of
        % the two choice options in the current trial (order depending on
        % choice):        
        if sub_choice(i) == 1
        ...
        % positive and negative PE, respectively)
        if sub_fb(i) == 1 % if fb is positive
            ...
            Q(unchosen_index) = Q(unchosen_index) + alpha_pos_unchosen*PE_u;
        ...
        elseif sub_fb(i) == 0 % if fb is negative
            ...
            Q(unchosen_index) = Q(unchosen_index) + alpha_neg_unchosen*PE_u;
