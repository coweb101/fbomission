% Model 1: Fit model with one global learning rate & an exploration
% parameter; only updates of chosen action value

addpath(genpath('./interim_datasets/'));

% load data (created with dataprep)
load datastruc_choice;
load datastruc_not_choice;
load datastruc_feedback;
load datastruc_stimuli;
...
for i = 1:nsubs
    ...
    for j = 1:niter
        ...
        alpha = rand; % one global learning rate
        ...
        % call fmincon to minimize output of function: probfb_fun_model1
        [params, LL] = fmincon(@probfb_fun_model1,params,[],[],[],[],LB,UB,[],options,......
