function error = probfb_fun_model4c(params,sub_choice,sub_fb,sub_not_choice,ntrials, sub_stimuli, sub_feedback_type)

% Separate learning rates for reward and loss context, and chosen and
% unchosen action (+ update of unchosen)

% set parameters
alpha_rewardcontext = params(1);
alpha_losscontext = params(2);
alpha_rewardcontext_unchosen = params(3);
alpha_losscontext_unchosen = params(4);
beta = params(5);
...
for i = 1:ntrials
    ...
    if isnan(sub_choice(i)) || isnan(sub_fb(i))
        ...
        if sub_feedback_type(i) == 1 & sub_fb(i) == 1 | sub_feedback_type(i) == 0 & sub_fb(i) == 0
            ...
            Q(unchosen_index) = Q(unchosen_index) + alpha_rewardcontext_unchosen*PE_u;
        
        else
            
            % use alpha_losscontext to update action value of chosen
            Q(chosen_index) = Q(chosen_index)+ alpha_losscontext*PE_c;
            Q(unchosen_index) = Q(unchosen_index) + alpha_losscontext_unchosen*PE_u;
